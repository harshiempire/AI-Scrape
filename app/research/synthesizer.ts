...
export const SourceInfoSchema = z.object({
  ...
  content_type: z.enum(["article", "research_paper", "news", "blog", "documentation", "other"]),
...
export const SynthesizedInsightSchema = z.object({
  ...
  insight_type: z.enum(["fact", "opinion", "analysis", "trend", "prediction", "definition"]),
...
Provide a comprehensive JSON response with executive summary, key findings, detailed insights with source attributions, confidence assessments, and recommendations for further research.`;
  ...
  async synthesize_research(data: RawResearchData): Promise<ResearchSynthesis> {
    ...
    try {
      ...
      // Generate synthesis
      const synthesis = await this.generate_synthesis(data.query, context, sources);
      ...
      return {
        ...
        synthesis_metadata: {
          ...
          methodology: "LLM-powered multi-source synthesis with confidence scoring",
        },
      };
  ...
  private classify_content_type(domain: string, title: string): string {
    if (domain.includes("arxiv.org") || domain.includes("nature.com") || domain.includes("science.org")) {
  ...
  private create_synthesis_context(data: RawResearchData, sources: SourceInfo[]): string {
    let context = `Research Query: ${data.query}\n\n`;
    context += `Sources Summary (${sources.length} total):\n`;
    
    sources.forEach(source => {
      context += `- ${source.title} (${source.domain}) - Authority: ${source.authority_score}/10, Relevance: ${(source.relevance_score * 100).toFixed(0)}%\n`;
    });
  ...
  private async generate_synthesis(query: string, context: string, sources: SourceInfo[]): Promise<Omit<ResearchSynthesis, 'synthesis_metadata'>> {
    this.memory.clear();
    this.memory.add_message(Message.system_message(this.get_synthesis_prompt()));
  ...
  private create_structured_synthesis(query: string, sources: SourceInfo[], context: string): Omit<ResearchSynthesis, 'synthesis_metadata'> {
    ...
    key_findings.push(`Research covered ${sources.length} sources across ${Object.keys(source_types).length} different content types`);
    
    const avg_authority = sources.reduce((sum, s) => sum + s.authority_score, 0) / sources.length;
    key_findings.push(`Average source authority score: ${avg_authority.toFixed(1)}/10`);
    ...
    const confidence_assessment = {
      ...
      information_completeness: sources.length >= 5 ? 0.8 : sources.length / 5 * 0.8,
    };

    return {
      query,
      executive_summary: `Research analysis of "${query}" based on ${sources.length} sources reveals ${key_findings.length} key findings with an overall confidence score of ${(confidence_assessment.overall_confidence * 100).toFixed(0)}%. The analysis includes diverse source types and provides comprehensive coverage of the topic.`,
      key_findings,
      insights,
      sources,
      confidence_assessment,
      research_gaps: this.identify_research_gaps(sources, query),
      recommendations: this.generate_recommendations(sources, confidence_assessment),
    };
  }
  ...
  private identify_research_gaps(sources: SourceInfo[], query: string): string[] {
    const gaps: string[] = [];
    ...
    if (sources.filter(s => s.published_date && new Date(s.published_date) > new Date(Date.now() - 365 * 24 * 60 * 60 * 1000)).length < 2) {
  ...
  private generate_recommendations(sources: SourceInfo[], confidence: any): string[] {
    const recommendations: string[] = [];

    if (confidence.overall_confidence < 0.7) {
      recommendations.push("Seek additional high-authority sources to improve confidence");
    }
    ...
    if (!sources.some(s => s.content_type === "research_paper")) {
      recommendations.push("Include academic research papers for deeper analysis");
    }

    recommendations.push("Verify key findings with primary sources when possible");
    recommendations.push("Consider temporal aspects and recent developments in the field");
