...
export class ResearchMemory extends Memory {
  ...
  constructor(session_id?: string) {
    ...
    log.info(
      `Research memory initialized with session ID: ${this.research_state.session_id}`
    );
  }
  ...
  initialize_research(query: string, plan: ResearchPlan): void {
    ...
    this.research_state.context.research_phase = "planning";
  ...
  complete_subquery(subquery_id: string, insights: string[] = []): void {
    if (
      !this.research_state.context.completed_subqueries.includes(subquery_id)
    ) {
      this.research_state.context.completed_subqueries.push(subquery_id);
    }

    // Update research thread
    const thread = this.research_state.context.research_threads.find(
      (t) => t.thread_id === subquery_id
    );
    if (thread) {
  ...
  add_discovered_source(url: string, source_info: SourceInfo): void {
    ...
    log.debug(
      `Source discovered: ${url} (Authority: ${source_info.authority_score}/10)`
    );
  }
  ...
  add_validated_insight(insight: SynthesizedInsight): void {
    ...
    this.add_message(
      Message.system_message(
        `Insight validated: ${insight.insight_type} with confidence ${(
  ...
  private update_source_quality_metrics(): void {
    ...
    if (dated_sources.length > 0) {
      ...
      this.research_state.quality_metrics.temporal_coverage = Math.min(
        date_range / year_ms,
        1
      );
    }
  ...
  private update_accumulated_knowledge(insight: SynthesizedInsight): void {
    ...
    if (insight.insight_type === "fact" && insight.confidence_score > 0.8) {
      this.research_state.accumulated_knowledge.important_facts.push(
        insight.content
      );
    }
  ...
  private detect_contradictions(new_insight: SynthesizedInsight): void {
    ...
    for (const existing of existing_insights) {
      ...
      if (
        overlap &&
        this.are_potentially_contradictory(
          existing.content,
          new_insight.content
        )
      ) {
        const contradiction = {
          ...
          description: `Potential contradiction between insights: "${existing.content.substring(
            0,
            100
          )}..." vs "${new_insight.content.substring(0, 100)}..."`,
        };

        this.research_state.accumulated_knowledge.contradictions.push(
          contradiction
        );
  ...
  private are_potentially_contradictory(
    content1: string,
    content2: string
  ): boolean {
    // Simple heuristic for detecting contradictions
    const negative_indicators1 = [
      "not",
      "no",
      "never",
      "cannot",
      "impossible",
      "false",
    ].some((word) => content1.toLowerCase().includes(word));
    const negative_indicators2 = [
      "not",
      "no",
      "never",
      "cannot",
      "impossible",
      "false",
    ].some((word) => content2.toLowerCase().includes(word));
  ...
  get_research_progress(): {
    ...
    return {
      ...
      completion_percentage:
        total_subqueries > 0 ? (completed / total_subqueries) * 100 : 0,
...
- Progress: ${progress.completion_percentage.toFixed(1)}% (${
      progress.completed_subqueries
    }/${progress.total_subqueries} subqueries)
...
- Contradictions: ${
      this.research_state.accumulated_knowledge.contradictions.length
    }`;
  }
  ...
  get_context_for_llm(): string {
    const progress = this.get_research_progress();
    const key_concepts =
      this.research_state.accumulated_knowledge.key_concepts.slice(0, 10);
...
Recent High-Confidence Insights: ${recent_insights
      .map((i) => i.content.substring(0, 100))
      .join("; ")}

Current Research Focus: ${
      this.research_state.context.current_subquery || "General research"
    }`;
  }
  ...
  cleanup_old_data(days: number = 7): void {
    ...
    if (session_start < cutoff) {
      ...
      log.info(
        `Archiving old research session: ${this.research_state.session_id}`
      );
