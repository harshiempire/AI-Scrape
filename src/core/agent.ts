import { LLM, GenerateParams, LLMOutput, ChatMessage, Memory } from "./types";
...
export class Agent {
  ...
  public onStep?: (step: AgentStep) => void;
  ...
  on(event: "step", callback: (step: AgentStep) => void): void {
    if (event === "step") {
      this.onStep = callback;
    }
  }

  private logStep(type: AgentStep["type"], data: any) {
  ...
  private parseToolCall(response: string): { tool: string; input: any } | null {
  ...
  async chat(userMessage: string): Promise<{ text: string }> {
    ...
    if (toolCall) {
      ...
      try {
        ...
        await this.memory.store({
          role: "assistant",
          content: `Tool call: ${toolCall.tool}(${JSON.stringify(
            toolCall.input
        ...
        const finalPrompt = await this.buildPrompt(
          `Tool ${toolCall.tool} returned: ${JSON.stringify(
            toolResult
