...
async function createResearchAgent() {
  ...
  const researchAgent = new Agent({
    ...
    tools: registry,
    system:
      "You are a research assistant. Use the search tool to find information and provide comprehensive summaries.",
    debug: true,
  });
...
async function runExample() {
  ...
  const calcResponse = await agent.chat(
    "Calculate (2+3)*7 using the calculator tool"
  );
  console.log("Response:", calcResponse.text);
  console.log();

  console.log("🌤️ Example 2: Weather Tool");
  const weatherResponse = await agent.chat(
  ...
  const researchResponse = await agent.chat(
    "Research information about artificial intelligence"
  );
