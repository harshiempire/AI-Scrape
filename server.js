...
import { Agent, ToolRegistry, InMemoryMemory, OpenAILLM, } from "./src/core/index.js";
...
try {
    defaultAgent = new Agent({
        ...
        tools: registry,
        system: "You are a helpful AI assistant with access to various tools. Use the available tools when appropriate to help users with their requests.",
        debug: true,
    });
    activeAgents.set("default", defaultAgent);
}
catch (error) {
    console.warn("⚠️  OpenAI API key not found. Running in demo mode with mock responses.");
}
// API Routes
// Get available tools
...
server.listen(PORT, () => {
    ...
    console.log(`🔧 Available tools: ${registry
        .list()
        .map((t) => t.name)
        .join(", ")}`);
