 ...
 const connectWebSocket = () => {
  ...
  const loadConversation = async () => {
    try {
      ...
      if (stored) {
        const conv: Conversation = JSON.parse(stored);
  ...
  const handleSendMessage = async () => {
    if (!inputText.trim() || isLoading || !wsConnection) return;
  ...
  const renderToolCall = (toolCall: ToolCall) => {
    ...
    return (
      <View key={toolCall.id} style={styles.toolCallContainer}>
  ...
  const renderMessage = ({ item }: { item: Message }) => (
    <View style={[
      ...
      <View style={styles.messageHeader}>
        <Ionicons 
          name={item.role === 'user' ? 'person' : 'hardware-chip'} 
          size={16} 
          color={item.role === 'user' ? '#6366f1' : '#10b981'} 
        />
  ...
  return (
    ...
    >
      <View style={styles.chatContainer}>
        ...
        {currentToolCall && (
          <View style={styles.currentToolCall}>
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            ...
            <TouchableOpacity
              ...
              onPress={() => setShowToolSelector(false)}
            >
