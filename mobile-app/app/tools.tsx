...
export default function ToolsScreen() {
  ...
  const saveToolStates = async (updatedTools: Tool[]) => {
    try {
  ...
  const resetAllTools = () => {
    Alert.alert(
      'Reset All Tools',
      'This will reset all tool states to their defaults. Continue?',
      [
  ...
  const testTool = (tool: Tool) => {
    Alert.alert(
      'Test Tool',
      `Testing ${tool.name}...\n\nThis would normally connect to your AI Agents backend to execute the tool.`,
      [{ text: 'OK' }]
    );
  };
    ...
    >
      <View style={styles.toolHeader}>
        <View style={styles.toolIcon}>
          <Ionicons 
            name={item.category === 'Math' ? 'calculator' : 'search'} 
            size={24} 
            color="#6366f1" 
          />
        </View>
        ...
        <View style={styles.toolControls}>
          <View style={styles.toolStatus}>
            <Ionicons 
              name={getStatusIcon(item.enabled)} 
              size={20} 
              color={getStatusColor(item.enabled)} 
            />
          </View>
          <Switch
            value={item.enabled}
            ...
            thumbColor={item.enabled ? '#fff' : '#f4f4f4'}
          />
        </View>
      </View>
